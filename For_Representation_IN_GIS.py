...
for i in arcpy.ListFiles("*.xls"): 
	...
	list.append([sheet.cell(19,7)])
	arcpy.AddMessage (sheet.cell(19,7).value) 
	arcpy.AddMessage(arcpy.ListFiles("*.shp")[counterXLS-1])
	arcpy.AddMessage ("final OutPut Path454")
	arcpy.AddMessage (outputFolderPath+str(sheet.cell(19,7).value)+arcpy.ListFiles("*.shp")[counterXLS-1])
...
for i in range(0,len(afterRemovingDups)):
	...
	if len(tempListIndex)==1:
		...
		arcpy.CopyFeatures_management(arcpy.ListFiles("*.shp")[tempListIndex[0]], outputFolderPath+"_"+anyValue+"_"+tempListGroupNameString+"_"+list2[i])
	elif len(tempListIndex)>1:
		arcpy.AddMessage("> one value")
		arcpy.Merge_management('"'+';'.join(tempListName)+'"', outputFolderPath+"_"+anyValue+"_"+tempListGroupNameString+"_"+list2[i])
